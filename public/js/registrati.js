...
async function handleRegistrationSubmit(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            setTimeout(() => { window.location.href = '/accedi'; }, 2000);
        } else {
...
function checkPasswordStrength(password) {
    ...
    if (strength <= 2) return { color: 'danger', text: 'Debole', percent: Math.max(percent, 10) };
    if (strength <= 3) return { color: 'warning', text: 'Media', percent };
