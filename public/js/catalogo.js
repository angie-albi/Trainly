...
async function loadProducts() {
    try {
        ...
        if (result && result.success && Array.isArray(result.data)) {
            ...
            displayProducts(); // usa filteredProducts di default
            updateProductCount();
            toggleShowAllButton();
        } else {
...
// ---------- CREATE CARD (markup coerente con il CSS) ----------
    ...
    <div class="col product-item">
      <div class="card h-100 shadow-sm product-card" data-product-id="${id}" data-category="${product.category || ''}" data-price="${price}">
        ...
        <div class="card-footer bg-transparent">
          ...
          <button class="btn w-100 btn-carrello-aggiungi btn-costum text-white" data-id="${id}" aria-label="Aggiungi ${escapeHtml(title)} al carrello">
...
// escape semplice per sicurezza nel markup
...
function setupEventListeners() {
    // Filtri categoria (link con data-category-filter)
...
// Modifica la funzione setupResetFilters
function setupResetFilters() {
    // Crea il pulsante per desktop (sidenav)
    ...
    // Crea il pulsante per mobile (offcanvas filtri)
    ...
    if (mobileOffcanvas && !mobileOffcanvas.querySelector('#show-all-btn-mobile')) {
        ...
        wrapper.innerHTML = `
            <button id="show-all-btn-mobile" class="btn btn-custom text-white d-none w-100">
                Mostra tutti
            </button>
        `;
...
// Modifica la funzione toggleShowAllButton per gestire entrambi i pulsanti
...
// SEARCH setup (collega input con class .form-search o input[type="search"])
...
// PRICE filters wiring (icona o bottoni)
function setupPriceFilters() {
    ...
    // Enter su input mobile e desktop
...
function addCartButtonListeners() {
    // seleziono la classe usata nelle card: btn-carrello-aggiungi
    document.querySelectorAll('.btn-carrello-aggiungi, .add-to-cart-btn').forEach(button => {
        ...
        button.addEventListener('click', function(e) {
            ...
            // prova entrambe le possibili funzioni globali del carrello
